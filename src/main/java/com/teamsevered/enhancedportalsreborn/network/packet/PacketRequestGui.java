...
//        buffer.writeBytes(g.name().getBytes());
    ...
    {
        t = player.world.getTileEntity(new BlockPos(x, y, z));

        if (t != null)
        {
            player.openGui(EnhancedPortalsReborn.instance, g.ordinal(), t.getWorld(), x, y, z);
        }
        else
        {

            player.openGui(EnhancedPortalsReborn.instance, g.ordinal(), Minecraft.getMinecraft().world, (int) player.posX, (int) player.posY, (int) player.posZ);
        }
    }
}
