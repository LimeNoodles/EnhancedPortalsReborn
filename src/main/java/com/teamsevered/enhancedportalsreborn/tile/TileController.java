...
import com.teamsevered.enhancedportalsreborn.network.packet.PacketRerender;
import com.teamsevered.enhancedportalsreborn.portal.*;
import com.teamsevered.enhancedportalsreborn.util.*;
...
//   @Override
    public boolean activate(EntityPlayer player, ItemStack stack)
    {
        ...
        try
        {
            if (stack != null)
            {
                ...
                {
                    if (stack.getItem() == ItemLocationCard.instance && !world.isRemote)
                    {
                        boolean reconfiguring = dimensionalBridgeStabilizer != null;
                        setDBS(player, stack);
                        configurePortal();
                        player.sendMessage(new TextComponentString(Localization.getChatSuccess(!reconfiguring ? "create" : "reconfigure")));
                    }
                ...
                {
                    if (GeneralUtils.isWrench(stack) && !world.isRemote)
                    {
                        configurePortal();
                        player.sendMessage(new TextComponentString(Localization.getChatSuccess("reconfigure")));
                    }
                ...
                else if (portalState == ControlState.FINALIZED)
                {
                    ...
                    {
                        GuiHandler.openGui(player, this, GuiEnums.GUI_CONTROLLER.CONTROLLER_A.ordinal());
                        return true;
                    }
                    else if (stack.getItem() == ItemNanobrush.instance)
                    {
                        GuiHandler.openGui(player, this, GuiEnums.GUI_TEXTURE.TEXTURE_A.ordinal());
                        return true;
                    }
                }
            }
    ...
    public void addDataToPacket(NBTTagCompound tag)
    {
        ...
        if (isPortalActive())
        {
            ...
            tag.setInteger("destX", cachedDestinationLoc.x);
            tag.setInteger("destZ", cachedDestinationLoc.z);
        ...
        {
            tag.setInteger("ModX", moduleManipulator.x);
            tag.setInteger("ModZ", moduleManipulator.z);
        }
    }
  ...
  /*  @Override
    ...
    void configurePortal() throws PortalException
    {
        ...
        for (ChunkPos c : portalStructure)
        {
            ...
            //todoelse if (tile instanceof TileTransferFluid)
            //{
        ...
        markDirty();
//   todo     world.markBlockForUpdate(xCoord, yCoord, zCoord);
    ...
    {
        if (world.isRemote || getIdentifierNetwork() == null)
        {
            return;
        }
        ...
        try
        {
            ...
            dbs.setupNewConnection(getIdentifierUnique(), EnhancedPortalsReborn.proxy.networkManager.getDestination(getIdentifierUnique(), getIdentifierNetwork()), null);
        }
        catch (PortalException e)
        {
            EntityPlayer player = world.getClosestPlayer(getPos().getX() + 0.5, getPos().getY() + 0.5, getPos().getZ() + 0.5, 128, false);
    ...
    public void connectionDial(GlyphIdentifier id, PortalTextureManager m, EntityPlayer player)
    {
        ...
        catch (PortalException e)
        {
            if (player == null)
            {
                player = world.getClosestPlayer(getPos().getX() + 0.5, getPos().getY() + 0.5, getPos().getZ() + 0.5, 128, false);
            }
    ...
    public void deconstruct()
    {
        ...
        {
            TileEntity t = world.getTileEntity(getPos());
...
//            todo TileEntity tile = w.getTileEntity(dimensionalBridgeStabilizer.posX, dimensionalBridgeStabilizer.posY, dimensionalBridgeStabilizer.posZ);
    ...
    public GlyphIdentifier getIdentifierNetwork()
    {
        ...
        return EnhancedPortalsReborn.proxy.networkManager.getPortalNetwork(getIdentifierUnique());
    }
    ...
    public GlyphIdentifier getIdentifierUnique()
    {
        ...
        return EnhancedPortalsReborn.proxy.networkManager.getPortalIdentifier(getDimensionCoordinates());
    }
    ...
    public TilePortalManipulator getModuleManipulator()
    {
        if (moduleManipulator != null)
        {
//            TileEntity tile = world.getTileEntity(moduleManipulator.posX, moduleManipulator.posY, moduleManipulator.posZ);
            TileEntity tile = world.getTileEntity(getPos());
    ...
    public void onDataPacket(NBTTagCompound tag)
    {
        ...
        ArrayList<ChunkPos> f = EnhancedPortalsReborn.proxy.getControllerList(getChunkPos());
        ...
        if (f != null)
        {
            for (ChunkPos frames : f)
            {
                ...
                //todo markBlockForUpdate
//                world.markBlockForUpdate(frames.posX, frames.posY, frames.posZ);
                final IBlockState state = getWorld().getBlockState(getPos());
                world.notifyBlockUpdate(getPos(), state, state, 3);
            }

            EnhancedPortalsReborn.proxy.clearControllerList(getChunkPos());
        }
    }
    ...
    public void onEntityEnterPortal(Entity entity, TilePortal tilePortal)
    {
        ...
        if (tile != null && tile instanceof TileController)
        {
            ...
            catch (PortalException e)
            {
                ...
                {
                    ((EntityPlayer) entity).sendMessage(new TextComponentString(e.getMessage()));
                }
            }
        }
    ...
    public void onEntityTouchPortal(Entity entity)
    {
        ...
        {
            ((TileRedstoneInterface) world.getTileEntity(getPos())).onEntityTeleport(entity);
        }
    }
    ...
    public void portalCreate() throws PortalException
    {
        ...
        for (ChunkPos c : portalBlocks)
        {
            //todo setBlock
            final IBlockState state = getWorld().getBlockState(getPos());
//            world.setBlockState(getPos(), state, portalType, 2);
            world.setBlockState(getPos(), state);

            TilePortal portal = (TilePortal) world.getTileEntity(getPos());
            portal.portalController = getChunkPos();
        }

        for (ChunkPos c : getRedstoneInterfaces())
        {
            TileRedstoneInterface ri = (TileRedstoneInterface) world.getTileEntity(getPos());
            ri.onPortalCreated();
        }
    }
    ...
    void sendUpdatePacket(boolean updateChunks)
    {
        ...
        if (updateChunks)
        {
            ...
            {
                if (!chunks.contains(new ChunkPos(c.x >> 4, c.z >> 4)))
                {
                    EnhancedPortalsReborn.packetPipeline.sendToAllAround(new PacketRerender(getPos()), this);
                    chunks.add(new ChunkPos(c.x >> 4, c.z >> 4));
                }
            }
        }
    }
    ...
    void setDBS(EntityPlayer player, ItemStack stack) throws PortalException
    {
        ...
        else if (!stabilizer.equals(getDimensionalBridgeStabilizer()))
        {
            if (!player.capabilities.isCreativeMode)
            {
                ...
                if (stack.getCount() <= 0)
                {
                   //todo player.inventory.mainInventory[player.inventory.currentItem] = null;
                }
            }
    ...
    public void setIdentifierNetwork(GlyphIdentifier id)
    {
        ...
        {
            EnhancedPortalsReborn.proxy.networkManager.removePortalFromNetwork(uID, getIdentifierNetwork());
        }

        if (id.size() > 0)
        {
            EnhancedPortalsReborn.proxy.networkManager.addPortalToNetwork(uID, id);
        }
    ...
    {
        if (EnhancedPortalsReborn.proxy.networkManager.getPortalLocation(id) != null) // Check to see if we already have a portal with this ID
        {
        ...
        if (getHasIdentifierUnique()) // If already have an identifier
        {
            ...
            if (getHasIdentifierNetwork()) // Check to see if it's in a network
            {
                ...
                EnhancedPortalsReborn.proxy.networkManager.removePortalFromNetwork(getIdentifierUnique(), networkIdentifier); // Remove it if it is
            }

            EnhancedPortalsReborn.proxy.networkManager.removePortal(getDimensionCoordinates()); // Remove the old identifier

            if (id.size() > 0) // If the new identifier isn't blank
            {
                EnhancedPortalsReborn.proxy.networkManager.addPortal(id, getDimensionCoordinates()); // Add it

                if (networkIdentifier != null)
                {
                    EnhancedPortalsReborn.proxy.networkManager.addPortalToNetwork(id, networkIdentifier); // Re-add it to the network, if it was in one
                }
            }
        }
        else if (id.size() > 0)
        {
            EnhancedPortalsReborn.proxy.networkManager.addPortal(id, getDimensionCoordinates()); // Add the portal
        }
