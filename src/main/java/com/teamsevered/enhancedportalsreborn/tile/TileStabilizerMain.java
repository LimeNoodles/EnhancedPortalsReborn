...
//   todo     worldObj.markBlockForUpdate(xCoord, yCoord, zCoord);
    ...
    void setInstability(int newInstability)
    {
        ...
        {
            TileController controllerA = EnhancedPortalsReborn.proxy.networkManager.getPortalController(new GlyphIdentifier(pair.getKey()), getPos());
            TileController controllerB = EnhancedPortalsReborn.proxy.networkManager.getPortalController(new GlyphIdentifier(pair.getValue()), getPos());
    ...
    public boolean setupNewConnection(GlyphIdentifier portalA, GlyphIdentifier portalB, PortalTextureManager textureManager) throws PortalException
    {
        ...
        else if (!EnhancedPortalsReborn.proxy.networkManager.portalIdentifierExists(portalA))
        {
            throw new PortalException("noPortalWithThatIdentifierSending");
        }
        else if (!EnhancedPortalsReborn.proxy.networkManager.portalIdentifierExists(portalB))
        {
            throw new PortalException("noPortalWithThatIdentifierReceiving");
        }

        TileController cA = EnhancedPortalsReborn.proxy.networkManager.getPortalController(portalA, getPos()), cB = EnhancedPortalsReborn.proxy.networkManager.getPortalController(portalB, getPos());
