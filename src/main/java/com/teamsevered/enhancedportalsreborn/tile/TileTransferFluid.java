...
//            buffer.writeInt(tank.getFluid().getFluidID());
    ...
    public void packetGuiUse(ByteBuf buffer)
    {
        ...
        {
            //todo tank.setFluid(new FluidStack(FluidRegistry.getFluid(buffer.readInt()), buffer.readInt()));
        }
        else
        {
            tank.setFluid(null);
    ...
    void transferFluid(int side)
    {
        ...
        //todo tank.drain(handlers[side].fill(EnumFacing.getFront(side).getOpposite(), tank.getFluid(), true), true);
    }
    ...
    void updateFluidHandlers()
    {
        for (int i = 0; i < 6; i++)
        {
            ...
            if (tile != null && tile instanceof IFluidHandler)
            {
                ...
                if (fluid.getTankProperties() != null)
                {
                    handlers[i] = fluid;
                }
                else
