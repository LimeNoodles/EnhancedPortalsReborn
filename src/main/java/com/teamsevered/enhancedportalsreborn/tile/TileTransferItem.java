...
public class TileTransferItem extends TileFrameTransfer implements IInventory, SimpleComponent
{
    ...
    public boolean activate(EntityPlayer player, ItemStack stack)
    {
        ...
        {
            GuiHandler.openGui(player, this, GuiEnums.GUI_TRANFER.TRANSFER_ITEM.ordinal());
            return true;
        }
...
//    @Override
    ...
    public void readFromNBT(NBTTagCompound tag)
    {
        ...
        //TODO stack = ItemStack.loadItemStackFromNBT(tag.getCompoundTag("stack"));
    }
                                ...
                                if (tile != null && tile instanceof TileTransferItem)
                                {
                                    ...
                                    if (!item.isSending)
                                    {
                                        ...
                                        else if (item.getStackInSlot(0).getItem() == stack.getItem())
                                        {
                                            ...
                                            if (item.getStackInSlot(0).getCount() + stack.getCount() <= stack.getMaxStackSize())
                                            {
                                                amount = stack.getCount();
                                            }
                                            else
                                            {
                                                amount = stack.getCount() - (item.getStackInSlot(0).getCount() + stack.getCount() - 64);
                                            }
                                            ...
                                            item.getStackInSlot(0).setCount(amount);
                                            item.markDirty();
