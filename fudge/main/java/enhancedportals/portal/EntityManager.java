...
import enhancedportals.block.Blocks;
...
public class EntityManager
{
    ...
    static ChunkPos getActualExitLocation(Entity entity, TileController controller, BlockPos pos)
    {
        ...
        for (ChunkPos c : new ArrayList<ChunkPos>(controller.getPortals()))
        {
            ...
            {
                if (world.getBlockState(new BlockPos(pos.getX(), pos.getY() + i, pos.getZ())) != Blocks.PORTAL && !world.isAirBlock(new BlockPos(pos.getX(), pos.getY() + i, pos.getZ())));
                {
                    continue forloop;
                }
            }
...
*/
    ...
    {
        BlockPos spawn = par1Entity.world.getSpawnPoint();
//        spawn.posY = par1Entity.world.getTopSolidOrLiquidBlock(spawn.posX, spawn.posY);
//        spawn.getY() = par1Entity.world.getTopSolidOrLiquidBlock(par1Entity.world.getSpawnPoint().getY());

//        if (par1Entity.world.isAirBlock(new BlockPos(spawn.getX(), spawn.getY(), spawn.getZ())) || par1Entity.world.getBlock(spawn.posX, spawn.posY, spawn.posZ) instanceof BlockFluidBase)
        if (par1Entity.world.isAirBlock(new BlockPos(spawn.getX(), spawn.getY(), spawn.getZ())) || par1Entity.world.getBlockState(new BlockPos(spawn.getX(), spawn.getY(), spawn.getZ())) instanceof BlockFluidBase)
        {
            par1Entity.world.setBlockState(new BlockPos(spawn.getX(), spawn.getY(), spawn.getZ()), par1Entity.world.getBlockState(new BlockPos(spawn.getX(), spawn.getY(), spawn.getZ())));
        }
    ...
    static Entity transferEntity(Entity entity, BlockPos pos, double x, double y, double z, float yaw, WorldServer world, int touchedPortalType, int exitPortalType, boolean keepMomentum, int instability)
    {
        ...
        {
            return transferEntityToDimension(entity,pos, yaw, (WorldServer) entity.world, world, touchedPortalType, exitPortalType, keepMomentum, instability);
        }
    }
...
//            transferEntityWithRider(entity, pos, exitLoc.chunkXPos + 0.5, pos.getY(), exitLoc.chunkZPos + 0.5, getRotation(entity, exit, exitLoc), (WorldServer) exit.getWorld(), entry.portalType, exit.portalType, keepMomentum, instability);
    ...
    static Entity transferEntityToDimension(Entity entity, BlockPos pos, float yaw, WorldServer exitingWorld, WorldServer enteringWorld, int touchedPortalType, int exitPortalType, boolean keepMomentum, int instability)
    {
        ...
        else if (entity instanceof EntityPlayer)
        {
            ...
            player.world.profiler.startSection("portal");
            ...
            if (!player.world.isRemote)
            {
                ...
                player.connection.sendPacket(new SPacketRespawn(player.dimension, player.world.getDifficulty(), enteringWorld.getWorldInfo().getTerrainType(), player.interactionManager.getGameType()));
                ...
                enteringWorld.spawnEntity(player);
...
//                todo config.func_72375_a(player, exitingWorld);
        ...
        else
        {
            ...
            if (newEntity != null)
            {
                ...
                enteringWorld.spawnEntity(newEntity);
