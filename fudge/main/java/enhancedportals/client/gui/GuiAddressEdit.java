...
public class GuiAddressEdit extends GuiAddressBookAdd
{
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException
    {
        if (receivedData)
        {
            ...
            {
                isEditing = true;
                EnhancedPortals.packetPipeline.sendToServer(new PacketRequestGui(player, GuiEnums.GUI_ADDRESS_BOOK.ADDRESS_BOOK_E));
            }
        }
    }
    ...
    protected void actionPerformed(GuiButton button)
    {
        if (button.id == 0)
        {
            try
            {
                EnhancedPortals.packetPipeline.sendToServer(new PacketRequestGui(player, GuiEnums.GUI_ADDRESS_BOOK.ADDRESS_BOOK_A));
            }
            catch (Exception e)
            {
