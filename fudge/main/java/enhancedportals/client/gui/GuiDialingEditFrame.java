...
public class GuiDialingEditFrame extends GuiTextureFrame
{
    ...
    protected void actionPerformed(GuiButton button)
    {
        ...
        {
            didSave = true;
            EnhancedPortals.packetPipeline.sendToServer(new PacketRequestGui(dial, returnToEdit ? GuiEnums.GUI_DIAL.DIAL_D : GuiEnums.GUI_DIAL.DIAL_C));
        }
        else if (button.id == 500)
        {
            didSave = true;
            EnhancedPortals.packetPipeline.sendToServer(new PacketRequestGui(dial, returnToEdit ? GuiEnums.GUI_TEXTURE.TEXTURE_DIAL_EDIT_B : GuiEnums.GUI_TEXTURE.TEXTURE_DIAL_SAVE_B));
        }
        else if (button.id == 501)
        {
            didSave = true;
            EnhancedPortals.packetPipeline.sendToServer(new PacketRequestGui(dial, returnToEdit ? GuiEnums.GUI_TEXTURE.TEXTURE_DIAL_EDIT_C : GuiEnums.GUI_TEXTURE.TEXTURE_DIAL_SAVE_C));
        }
    }
